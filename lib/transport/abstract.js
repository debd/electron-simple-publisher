...
const extractAuthFromUrl = require('../utils/extract-auth-from-url');
...
class AbstractTransport extends EventEmitter {
  ...
  fetchUpdatesJson() {
    ...
    return new Promise((resolve) => {
      const parsedUrl = extractAuthFromUrl(this.getUpdatesJsonUrl());

      http.get(parsedUrl.url, parsedUrl.options, (err, res) => {
        if (err) {
          console.warn(
